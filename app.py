...
def build_kb():
    ...
    if not os.path.exists(folder) or len(os.listdir(folder)) == 0:
        st.error("📂 Please upload PDFs inside /data folder")
        st.stop()
    ...
    atomic_write_pickle(CHUNKS_PATH, chunks)
    atomic_write_json(META_PATH, {"chunks": len(chunks)})
...
def load_or_build():
    try:
        if not (
            ...
            st.info("📦 No stored index found. Building...")
            return build_kb()
        ...
        # 🔥 CRITICAL FIX: FAISS DIMENSION VALIDATION
        test_vec = embedder.encode(["test"]).astype("float32")

        if test_vec.shape[1] != index.d:
            st.warning("⚠️ FAISS mismatch detected. Rebuilding index...")
            return build_kb()
        ...
        st.warning(f"⚠️ Storage corrupted. Rebuilding... ({str(e)})")
        return build_kb()
